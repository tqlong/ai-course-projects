        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                if (nextx, nexty) in self.corners:
                    newCorners[self.corners.index((nextx, nexty))] = True
                successors.append(
                    (((nextx, nexty), tuple(newCorners)), action, 1))
...
def cornersHeuristic(state, problem):
    ...
    if problem.isGoalState(state):
        ...
        cornersLeft = [
            corner for corner in corners if not state[1][corners.index(corner)]]
        if len(cornersLeft) == 1:
            return manhattanDistance(state[0], cornersLeft[0])
        else:
            return max([manhattanDistance(state[0], corner) for corner in cornersLeft])
