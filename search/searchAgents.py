...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                cornersLeft = list(unvisitedCorners)
                if (nextx, nexty) in unvisitedCorners:
                    cornersLeft.remove((nextx, nexty))
                successors.append(
                    (((nextx, nexty), tuple(cornersLeft)), action, 1))
...
def cornersHeuristic(state, problem):
    ...
    if problem.isGoalState(state):
        ...
        if len(unvisitedCorners) == 1:
            return util.manhattanDistance(currentPosition, unvisitedCorners[0])
        else:
            return max([util.manhattanDistance(currentPosition, corner)
