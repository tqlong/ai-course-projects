...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        if len(food_list) > 0:
            min_food = min([manhattanDistance(newPos, food)
    ...
    def getAction(self, gameState):
        ...
        def max_value(gameState, depth, agentIndex):
            ...
            for action in gameState.getLegalActions(agentIndex):
                v = max(v, minimax(gameState.generateSuccessor(
        ...
        def min_value(gameState, depth, agentIndex):
            ...
            for action in gameState.getLegalActions(agentIndex):
                if agentIndex == gameState.getNumAgents() - 1:
                    v = min(v, minimax(gameState.generateSuccessor(
    ...
    def getAction(self, gameState):
        ...
        def min_value(gameState, depth, agentIndex, alpha, beta):
            ...
            for action in gameState.getLegalActions(agentIndex):
                if agentIndex == gameState.getNumAgents() - 1:
                    v = min(v, minimax(gameState.generateSuccessor(
                        agentIndex, action), depth - 1, 0, alpha, beta))
                else:
                    v = min(v, minimax(gameState.generateSuccessor(
                        agentIndex, action), depth, agentIndex + 1, alpha, beta))
                if v < alpha:
...
class ExpectimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def max_value(gameState, depth, agentIndex):
            v = float("-inf")
            for action in gameState.getLegalActions(agentIndex):
                v = max(v, expectimax(gameState.generateSuccessor(
                    agentIndex, action), depth, agentIndex + 1))
            return v
        ...
        def exp_value(gameState, depth, agentIndex):
            ...
            for action in gameState.getLegalActions(agentIndex):
                if agentIndex == gameState.getNumAgents() - 1:
                    v += expectimax(gameState.generateSuccessor(agentIndex,
                                    action), depth - 1, 0)
                else:
                    v += expectimax(gameState.generateSuccessor(agentIndex,
        ...
        bestAction = None
        for action in gameState.getLegalActions(0):
            temp = expectimax(gameState.generateSuccessor(
                0, action), self.depth, 1)
            if temp > v:
                v = temp
                bestAction = action
...
def betterEvaluationFunction(currentGameState):
    ...
    distFood = [manhattanDistance(pacmanPos, food) for food in foodList]
    if len(distFood) > 0:
        score += food_value / min(distFood)
    else:
        score += food_value
    for ghost in ghostStates:
        distGhost = manhattanDistance(pacmanPos, ghost.getPosition())
        if ghost.scaredTimer > 0:
            score += scared_ghost_value / distGhost
        else:
            if distGhost > 0:
                score += ghost_value / distGhost
            else:
                score += ghost_value
    return score
